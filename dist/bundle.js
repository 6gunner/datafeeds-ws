    ...
    var Requester = function () {
      function Requester(url) {
        ...
        ws.on('open', function (event) {
          _.each(_this._callbacks, function (callback, callbackKey) {
        ...
        ws.on('message', function (event) {
          ...
          if (data.id > 0 && _this._callbacks[data.id]) {
            var callback = _this._callbacks[data.id].callback;
            callback(event, data);
          } else {
            var func = _this._callbacks[data.api + '#' + data.topic];
            if (func) {
              func.callback(data);
            }
          }
    ...
    var WSCompatibleDatafeedBase = function () {
      ...
      createClass(WSCompatibleDatafeedBase, [{
        ...
        value: function subscribeBars(symbolInfo, resolution, onRealtimeCallback, subscriberUID, onResetCacheNeededCallback) {
          ...
          resolution = getPeriodByInterval(resolution);
          var topic = symbolInfo.exchange + '.' + symbolInfo.ticker + '.' + resolution;
